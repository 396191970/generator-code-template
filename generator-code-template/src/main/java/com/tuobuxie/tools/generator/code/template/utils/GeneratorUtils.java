...
import com.tuobuxie.tools.generator.code.template.entity.ColumnEntity;
import com.tuobuxie.tools.generator.code.template.entity.TableEntity;
...
public class GeneratorUtils {
    ...
    public static List<String> getTemplates() {
        ...
        // 这里改成你要遍历的目录路径
        String path = FileUtils.class.getClassLoader().getResource(TEMPLATE).getPath();
        FileUtils.recursiveFiles(path, fileList);

        for (File file : fileList) {

            String path1 = file.getPath();
            if (!path1.endsWith(".ignore")) {
                fileNameList.add(path1.substring(path.length()));
            }
        }
    ...
    public static void generatorCode(Map<String, String> table,
        ...
        for (String template : templates) {
            ...
            Template tpl = Velocity.getTemplate(TEMPLATE + File.separator + template, "UTF-8");
            tpl.merge(context, sw);

            try {
                //添加到zip
                String fileName = getFileName(template, tableEntity.getClassName(), config.getString("package"), config.getString("mainModule"));
